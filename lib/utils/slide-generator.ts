...
export function generateSlides(
  ...
  collaborators.forEach((collaborator, index) => {
    ...
    const shouldAddStatistic =
      (index + 1) % finalConfig.collaboratorsPerStatistic === 0 &&
...
export function getSlidesInfo(slides: SlideType[]) {
  const collaboratorCount = slides.filter(
    (s) => s.type === 'collaborator'
  ).length;
  const statisticCount = slides.filter((s) => s.type === 'statistic').length;
  const firefightersCount = slides.filter(
    (s) => s.type === 'firefighters'
  ).length;
